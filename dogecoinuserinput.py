...
loaded_model = tf.keras.models.load_model('dogecoin_price_predictor.h5')
...
if user_input_date <= pd.to_datetime('today'):
    ...
    historical_data = doge_data.loc[doge_data.index <= user_input_date]['Close'].values
    ...
    # Print the predicted Dogecoin prices for the user-input future date
    print("Predicted Dogecoin prices for {}: {}".format(user_input_date.strftime('%Y-%m-%d'), predicted_prices[-1]))
