...
if user_input_date <= pd.to_datetime('today'):
    ...
    historical_data = ltc_data.loc[ltc_data.index <= user_input_date]['Close'].values
    ...
    # Print the predicted Litecoin prices for the user-input future date
    print("Predicted Litecoin prices for {}: {}".format(user_input_date.strftime('%Y-%m-%d'), predicted_prices[-1]))
