...
loaded_model = tf.keras.models.load_model('solana_price_predictor.h5')
...
if user_input_date <= pd.to_datetime('today'):
    ...
    historical_data = sol_data.loc[sol_data.index <= user_input_date]['Close'].values
    ...
    # Print the predicted Solana prices for the user-input future date
    print("Predicted Solana prices for {}: {}".format(user_input_date.strftime('%Y-%m-%d'), predicted_prices[-1]))
