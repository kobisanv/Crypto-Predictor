...
loaded_model = tf.keras.models.load_model('ethereum_price_predictor.h5')
...
if user_input_date <= pd.to_datetime('today'):
    ...
    historical_data = eth_data.loc[eth_data.index <= user_input_date]['Close'].values
    ...
    # Print the predicted Ethereum  prices for the user-input future date
    print("Predicted Ethereum prices for {}: {}".format(user_input_date.strftime('%Y-%m-%d'), predicted_prices[-1]))
